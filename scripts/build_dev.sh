...
while kill -0 $HELM_PID 2>/dev/null; do
  ...
  kubectl get pods -o wide | grep -E "(NAME|secret-sharer)" || echo "No pods found yet"
  echo "[INFO] Recent events:"
  kubectl get events --sort-by='.lastTimestamp' | tail -5 || echo "No events yet"
  echo "---"
  sleep 30
done
...
if [ $HELM_EXIT_CODE -eq 0 ]; then
  echo "[INFO] Helm deployment completed successfully."
  echo "[INFO] Final pod status:"
  kubectl get pods -o wide
else
  echo "[ERROR] Helm deployment failed with exit code $HELM_EXIT_CODE"
  echo "[ERROR] Checking for issues..."
  kubectl get pods -o wide
